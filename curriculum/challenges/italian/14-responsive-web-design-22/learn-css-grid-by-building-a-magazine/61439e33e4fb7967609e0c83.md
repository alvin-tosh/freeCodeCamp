...
Dopo i tre elementi `p` all'interno dell'elemento `.text` crea un elemento `blockquote`. All'interno di questo, aggiungi un elemento `hr`, un elemento `p` con l'attributo `class` impostato a `quote` e un secondo elemento `hr`.
...
L'elemento `blockquote` dovrebbe venire dopo i tre elementi `p`.
