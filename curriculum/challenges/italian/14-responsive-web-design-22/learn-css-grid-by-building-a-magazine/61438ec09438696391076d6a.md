...
Sotto il tuo elemento `.heading`, crea un nuovo elemento `section` con l'attributo `class` impostato su `text`. All'interno, crea un elemento `p` con l'attributo `class` impostato su `first-paragraph` e il testo seguente:
